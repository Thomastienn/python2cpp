...
class LLMFixResponse(BaseModel):
    """
    Response structure for LLM fallback parsing.
    
    This class encapsulates the response from the LLM when it is used to
    fix compile errors or provide alternative code representations.
    
    Attributes:
        code (str | None): The C++ code generated by the LLM (None if there is no change)
    """
