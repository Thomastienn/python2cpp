...
# LLMFix is imported lazily inside the /fix endpoint to speed up cold start
...
async def check_abuse_tracking(request: Request, call_next):
    ...
    try:
        ...
        if (
            abuse_tracker[client_ip]["violations"]
            >= SECURITY_CONFIG["RATE_LIMIT_ABUSE_THRESHOLD"]
        ):
            # Ban the IP for configured duration
            abuse_tracker[client_ip]["ban_until"] = (
                current_time + SECURITY_CONFIG["RATE_LIMIT_BAN_DURATION"]
            )
...
async def convert(request: Request, req: PyCodeRequest):
    ...
    try:
        ...
        security_logger.info(
            f"Conversion request from {client_ip}, code size: {len(req.pycode)} bytes"
        )
        ...
        security_logger.error(
            f"Conversion error from {client_ip}: {type(e).__name__}: {str(e)}"
        )
...
async def fix(request: Request, req: CppCodeRequest):
    ...
    try:
        ...
        # Log conversion attempt
        security_logger.info(
            f"Conversion request from {client_ip}, code size: {len(req.cppcode)} bytes"
        )

        # Lazy import to avoid loading google-genai on cold start
        from llm_fix import LLMFix

        cpp_final = LLMFix(req.cppcode)

        return {"fix_code": cpp_final, "status": "success"}
