import sys, io
import unittest
import ast
from py2c.core.processor import ExprParser
from py2c.core.structure import ReprVisitContext, VisitContext

ReprVisitContext.model_rebuild()
class TestProcessor(unittest.TestCase):
    def capture_output(self, func, *args):
        original = sys.stdout
        sys.stdout = io.StringIO()
        func(*args)
        output = sys.stdout.getvalue()
        sys.stdout = original
        return output

    def setUp(self):
        self.parser = ExprParser({})

    def tearDown(self):
        self.parser.debug.close()

    def test_handle_pyfunc(self):
        self.parser.visit(ast.parse("a_tuple = (1,2,3)").body[0], VisitContext(
            allow_print = False
        ))
        node = ast.parse("len(a_tuple)").body[0].value
        s = self.parser.repr.handle_pyfunc(node, ReprVisitContext(
            processor=self.parser,
            parser_ctx=VisitContext(
                allow_print = True
            )
        ))
        assert s == "tuple_size<decltype(a_tuple)>::value", f"{s!r}"
        
    def test_assign(self):
        node = ast.parse("a = 2").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "int a = 2;\n", f"{s!r}"
        
        # Reassign
        node = ast.parse("a = 3").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "a = 3;\n", f"{s!r}"

        node = ast.parse("b = 2.0").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "float b = 2.0;\n", f"{s!r}"

        node = ast.parse("x = \"hi\"").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "string x = \"hi\";\n", f"{s!r}"

        node = ast.parse("c = a").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "int c = a;\n", f"{s!r}"

        node = ast.parse("d = [1]").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "vector<int> d = {1};\n", f"{s!r}"

        node = ast.parse("e = [1.0,2.0,3.0]").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "vector<float> e = {1.0, 2.0, 3.0};\n", f"{s!r}"

        # Auto convert to pair if it's just 2 elements
        node = ast.parse("f = (1,2.0)").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "pair<int, float> f = {1, 2.0};\n", f"{s!r}"

        node = ast.parse("g = (1,2.0, \"hi\")").body[0]
        s = self.capture_output(self.parser.visit, node)
        assert s == "tuple<int, float, string> g = make_tuple(1, 2.0, \"hi\");\n", f"{s!r}"


if __name__ == '__main__':
    unittest.main()
